...
class GRAC():
		...
		if test is False:
			with torch.no_grad():
				...
				if np.random.uniform(0,1) < ceof:
					better_action = self.searcher.search(state, action, self.critic.Q1, batch_size=1, clip=self.cem_clip)
	...
	def train(self, replay_buffer, batch_size=100, writer=None, reward_range=20.0, reward_max=0, episode_step_max=100, reward_min=0, episode_step_min=1):
		...
		while True:
			...
			self.update_critic(critic_loss3)
			if critic_loss3_1 < critic_loss * self.loss_decay and critic_loss3_1 < critic_loss2_1 * self.loss_decay and torch.sqrt(critic_loss3_2) < torch.max(torch.mean(torch.abs(target_Q1)) * 0.01, torch.mean(torch.abs(reward))) and torch.sqrt(critic_loss3_2) < reward_max * 1.0:
                                        break
			if idi > 50:
				break
		critic_loss = F.mse_loss(current_Q1, target_Q_final)
		...
		if log_it:
			...
			writer.add_scalar('train_loss/targetQ_condition',torch.mean(torch.abs(target_Q1)) ,self.total_it)
